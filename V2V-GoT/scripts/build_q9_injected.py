...
neighbor's Q6 intent text, ready for re-inference without retraining.
...
def build_injected(q9_records, q6_gt, q6_llm, matching_mode, max_chars):
    ...
    result = []
    # examples: keyed by 'ego', 'cav1', 'any' — store first injected for each
    ...
    for raw in q9_records:
        ...
        try:
            ...
            _   = rec['conversations'][0]['value']   # must be a string
        ...
        rec['q9_neighbor_intent_source_q6_id']    = q6_id
        rec['q9_neighbor_intent_matching_mode']   = matching_mode
        rec['q9_neighbor_intent_preview']         = q6_text[:160]
        n_injected += 1
...
def print_examples(examples, matching_mode):
    labels = [
    ...
    print("=" * 72)
    print(f"Examples  [matching_mode={matching_mode}]")
    print("=" * 72)
    for key, label in labels:
        ...
        # Show last 500 chars of the modified prompt so the injection is visible
...
def print_stats(stats, out_path, dry_run):
    print()
    print("=== Statistics ===")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            'Augment a Q9 eval JSON with the neighbor Q6 LLM intent text '
    ...
    print(f"matching_mode={args.matching_mode}  max_chars={args.max_chars}")
    print()

    # --- build ------------------------------------------------------------
    result, stats, examples = build_injected(
        q9_records, q6_gt, q6_llm, args.matching_mode, args.max_chars
    )

    # --- print examples BEFORE writing (so dry_run is useful) -------------
    print_examples(examples, args.matching_mode)
    print_stats(stats, args.out, args.dry_run)
