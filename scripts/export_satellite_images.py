...
# Output settings
OUTPUT_ROOT = "sat_images"
IMAGE_SIZE = 512
ZOOM_LEVEL = 20
ZOOM_LEVEL_TRAIN_7_8 = 19
ROTATE_TO_HEADING = True
ROTATE_PAD_SCALE = math.sqrt(2.0) + 0.02
HEADING_OFFSET_DEG = 0

# Pose angle interpretation for 6D poses.
...
# Note: Esri World Imagery has usage limits and is not licensed for bulk download.
# For large-scale use, consider Mapbox, MapTiler, or OSM-based providers.
TILE_URL = "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"
TILE_SIZE = 256

# Optional on-disk cache for tiles
...
def pose6_to_matrix(pose):
    """
    Pose order assumption (V2V4Real/OpenCOOD):
...
def get_yaw_from_pose_only(data):
    ...
    return yaw_from_matrix(T_world)
...
def world_yaw_to_compass_heading(yaw_world_deg, R_fit):
    """
    Convert world-frame yaw (CCW from world X) to compass heading (CW from North).
    We first rotate the unit heading vector into ENU using R_fit, then convert
    ENU yaw (CCW from East) to compass heading.
    """
    yaw_rad = math.radians(yaw_world_deg)
    v_world = np.array([math.cos(yaw_rad), math.sin(yaw_rad)])
    v_enu = R_fit @ v_world
    yaw_enu = math.degrees(math.atan2(v_enu[1], v_enu[0]))
...
def fetch_satellite_image(lat, lon, zoom, size):
    ...
    for ty in range(tile_y0, tile_y1 + 1):
        ...
        for tx in range(tile_x0, tile_x1 + 1):
            tx_wrap = tx % max_tile
            tile = fetch_tile(zoom, tx_wrap, ty)
...
def rotate_to_heading(image, heading_deg):
    """
    Rotate north-up imagery so vehicle heading points up.
    PIL rotates CCW for positive angles.
    """
    return image.rotate(heading_deg + HEADING_OFFSET_DEG, resample=Image.BICUBIC, expand=False)
...
for split_name in SPLITS:
    zoom_level = ZOOM_LEVEL_TRAIN_7_8 if split_name in {"train_07", "train_08"} else ZOOM_LEVEL
    ...
    for scenario_dir in scenario_dirs:
        ...
        if os.path.exists(ego_yaml_path):
            ...
            ego_lat, ego_lon, _ = get_ego_latlon(ego_data)
            ego_dir = os.path.join(scenario_dir, EGO_AGENT_ID)
        ...
        for frame_id in frame_ids:
            ...
            for agent_dir in agent_dirs:
                ...
                agent_yaw_world_deg = get_yaw_from_pose_only(data)
                ...
                fetch_size = int(round(IMAGE_SIZE * ROTATE_PAD_SCALE))
                image = fetch_satellite_image(agent_lat, agent_lon, zoom_level, fetch_size)
                if ROTATE_TO_HEADING:
                    heading_deg = world_yaw_to_compass_heading(agent_yaw_world_deg, R_fit)
                    image = rotate_to_heading(image, heading_deg)
