...
class LateFusionDataset(basedataset.BaseDataset):
    ...
    def get_item_single_car(self, selected_cav_base):
        ...
        # MY_CODE
        # save lidar pose as gt planning results
        selected_cav_processed.update({'lidar_pose': selected_cav_base['params']['lidar_pose']})
        #print("selected_cav_processed['lidar_pose']: ", selected_cav_processed['lidar_pose'])
        #assert False
    ...
    def collate_batch_test(self, batch):
        ...
        for cav_id, cav_content in batch.items():
            ...
            # MY_CODE 0306
            # Also store the projected_lidar
            # lidars in ego car's coordinate system
            # Also store the lidar_pose
            lidar_pose_torch = torch.from_numpy(
                    np.array(cav_content['lidar_pose'])).float()
            projected_lidar_torch = torch.from_numpy(projected_lidar)
            projected_lidar_torch = torch.unsqueeze(projected_lidar_torch, 0)
            ...
            output_dict[cav_id].update({'object_bbx_center': object_bbx_center,
                                        ...
                                        'projected_lidar': projected_lidar_torch,
                                        'lidar_pose': lidar_pose_torch})
            #print("output_dict[cav_id]['projected_lidar'].shape: ", output_dict[cav_id]['projected_lidar'].shape)
            # [1, N, 4]
            #print("output_dict[cav_id]['lidar_pose']: ", output_dict[cav_id]['lidar_pose'])
            # [4, 4]
            #assert False
    ...
    def post_process(self, data_dict, output_dict, return_in_dict=False):
        ...
        """
        gt_box_tensor, gt_object_id_tensor, visible_gt_object_ids_dict, invisible_gt_object_ids_dict = self.post_processor.generate_gt_bbx(data_dict)

        if return_in_dict:
          pred_box_dict, pred_score_dict, pred_feature_dict, pred_early_feature_dict, spatial_features, spatial_features_2d, regression_map, classification_map = \
              self.post_processor.post_process_return_in_dict(data_dict, output_dict)
          return pred_box_dict, pred_score_dict, gt_box_tensor, gt_object_id_tensor, pred_feature_dict, pred_early_feature_dict, spatial_features, spatial_features_2d, regression_map, classification_map, visible_gt_object_ids_dict, invisible_gt_object_ids_dict
        else:
