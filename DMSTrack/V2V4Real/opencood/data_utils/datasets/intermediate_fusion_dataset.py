...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def post_process(self, data_dict, output_dict, return_in_dict=False):
        ...
        if return_in_dict:
          ...
          #for key in spatial_features.keys():    
              #print('key: ', key)
              #print('spatial_features.shape: ', spatial_features[key].shape)
              #print('spatial_features_2d.shape: ', spatial_features_2d[key].shape)
              #print('regression_map.shape: ', regression_map[key].shape)
              #print('classification_map.shape: ', classification_map[key].shape)
              # key:  ego
              # spatial_features.shape:  torch.Size([1, 256, 48, 128])
              # spatial_features_2d.shape:  torch.Size([1, 256, 48, 128])
              # regression_map.shape:  torch.Size([1, 14, 48, 128])
              # classification_map.shape:  torch.Size([1, 2, 48, 128])
          #assert False    
          return pred_box_dict, pred_score_dict, gt_box_tensor, gt_object_id_tensor, pred_feature_dict, pred_early_feature_dict, spatial_features, spatial_features_2d, regression_map, classification_map
        else:
